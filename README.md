...
Retrying happens two ways: immediately whenever some other connection on the same upstream frees a slot, same as for static servers, and on a 200ms backstop timer for the case nothing else ever does (there is no way to be notified the instant DNS resolves without patching nginx core). Both paths always retry against a current snapshot of the peer set.
