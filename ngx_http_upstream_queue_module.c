...
typedef struct {
    ...
    ngx_queue_t queue;
} ngx_http_upstream_queue_save_t;
...
static void ngx_http_upstream_queue_peer_free(ngx_peer_connection_t *pc, void *data, ngx_uint_t state) {
    ...
    if (ngx_queue_empty(&d->conf->save)) return;
    ngx_queue_t *q = ngx_queue_head(&d->conf->save);
    ngx_queue_remove(q);
    ngx_queue_insert_tail(&d->conf->free, q);
    ngx_http_upstream_queue_save_t *s;
    s = ngx_queue_data(q, ngx_http_upstream_queue_save_t, queue);
...
static ngx_int_t ngx_http_upstream_queue_peer_get(ngx_peer_connection_t *pc, void *data) {
    ...
    if (ngx_queue_empty(&d->conf->free)) return rc;
    ngx_queue_t *q = ngx_queue_head(&d->conf->free);
    ngx_http_upstream_queue_save_t *s = ngx_queue_data(q, ngx_http_upstream_queue_save_t, queue);
    ...
    s->request = r;
    s->timeout.data = r;
    s->timeout.handler = ngx_http_upstream_queue_timeout_handler;
    s->timeout.log = pc->log;
    ngx_add_timer(&s->timeout, d->conf->timeout);
    ngx_queue_remove(q);
    ngx_queue_insert_tail(&d->conf->save, q);
    return NGX_DONE;
}
...
static ngx_int_t ngx_http_upstream_queue_peer_init_upstream(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *uscf) {
    ...
    ngx_http_upstream_queue_save_t *s;
    if (!(s = ngx_pcalloc(cf->pool, sizeof(*s) * qscf->max))) { ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "!ngx_pcalloc"); return NGX_ERROR; }
    ngx_queue_init(&qscf->save);
    ngx_queue_init(&qscf->free);
    for (ngx_uint_t i = 0; i < qscf->max; i++) { ngx_queue_insert_head(&qscf->free, &s[i].queue); }
    return NGX_OK;
}
